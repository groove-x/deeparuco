...
if __name__ == "__main__":
    parser = ArgumentParser(description="DeepArUco v2 demo tool with YOLOX.")
    parser.add_argument("pic_path", help="input image path")
    parser.add_argument("out_path", help="path to store output")
    parser.add_argument(
        "-d", "--detector", help="marker detector model path", default="det_luma_bc_s/model.pt"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--conf_threshold", type=float, default=0.5, help="confidence threshold for detection"
    )
    ...
    # Load models
    detector = YOLOXDetector(model_size='s', num_classes=1)
    detector.model.load_state_dict(torch.load(args.detector))
    detector.model.eval()
    ...
    # Detect markers
    detections = detector.predict(pic, conf_threshold=args.conf_threshold)
